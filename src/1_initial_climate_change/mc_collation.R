...
variants <- list.dirs(
    "../../southafrica_paper/S_Benguela_MA/",
    recursive = FALSE,
    full.names = FALSE
)
...
for (var in variants) {
    ...
    files_to_copy <- files[
        str_detect(files, pattern = "parameters") |
            str_detect(files, pattern = "mass") |
            str_detect(files, pattern = "AAM")
    ]
...
setwd(
    "C:/users/grier/OneDrive - University of Strathclyde/Documents/repos/southafrica-se2e2climate/"
)
...
for (variant in variants) {
    model <- e2e_read(
        "S_Benguela_MA",
        variant,
        models.path = "../../../../Documents/southafrica_paper/",
        results.path = out_dir,
        model.ident = str_glue("{variant}-MC")
    )
    cum_files <- list.files(
        file.path(out_dir, "S_Benguela_MA", variant, "CredInt"),
        full.names = TRUE,
        pattern = "cumulative"
    )

    inshore_aam_store <- cum_files[str_detect(
        cum_files,
        "cumulative_inshoreaamass"
    )]
    inshore_aam_store <- read.csv(inshore_aam_store)

    offshore_aam_store <- cum_files[str_detect(
        cum_files,
        "cumulative_offshoreaamass"
    )]
    offshore_aam_store <- read.csv(offshore_aam_store)

    whole_aam_store <- cum_files[str_detect(
        cum_files,
        "cumulative_wholeaamass"
    )]
    whole_aam_store <- read.csv(whole_aam_store)

    StrathE2E2:::CredInt_make_aamass_results(
        model,
        inshore_aam_store,
        offshore_aam_store,
        whole_aam_store,
