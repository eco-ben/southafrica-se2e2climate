...
sapply(variants, function(x) {
    dir.create(file.path(results_path_base, x, "demfish_sens"))
})
...
base_params <- sapply(
    ...
    function(esm) {
        model_e <- e2e_read(
            "S_Benguela_MA",
            str_glue("2010-2019-{esm}-ssp126"),
            models.path = models_path
        )
        sapply(params, function(x) {
            model_e$data$fitted.parameters[[x]]
        })
    }
)
...
write.csv(
    base_params_save,
    "./outputs/initial_runs/demfish_sens_base_params.csv",
    row.names = FALSE
)
...
sampled_params <- lapply(
    ...
    function(esm) {
        ...
        lapply(params, function(x) {
            base_val <- base_p[x]
            sampled_p <- seq(
                base_val * (1 - variation),
                base_val * (1 + variation),
                length.out = steps
            )
            return(data.frame(
                param_name = rep(x, steps),
                param_value = sampled_p
            ))
        })
    }
)
sampled_params <- lapply(sampled_params, function(x) {
    do.call(rbind, x)
})
for (i in 1:length(sampled_params)) {
    sampled_params[[i]]$esm <- esms[i]
}

param_runs <- do.call(rbind, sampled_params)
param_runs$param_id <- rep(seq(1, length(params) * steps), 2)
param_runs <- pivot_wider(
    param_runs,
    id_cols = c(param_name, param_id),
    names_from = esm,
...
param_run <- function(variant, param_row) {
    ...
    model_s <- e2e_read("S_Benguela_MA", variant, models.path = models_path)
    ...
    write.csv(
        ...
        file.path(
            ...
            variant,
            "demfish_sens",
            str_glue("initial_conditions_dfishsens_p{param_row$param_id}.csv")
        )
    )
    ...
    write.csv(
        ...
        file.path(
            ...
            variant,
            "demfish_sens",
            str_glue("final_year_dfishsens_p{param_row$param_id}.csv")
        )
    )
}
