...
for (esm_ssp in ESM_SSP) {
    ...
    shapley_results <- expand.grid(
        variable_group = variable_group_names,
        output = c(target_guilds, "netprimprod"),
        stringsAsFactors = FALSE
    )
    shapley_results$shapley_effect <- NA
    permutation_plan <- read.csv(
        paste0(sub_output_path, "permutation_plan.csv"),
        col.names = c(variable_group_names, "perm_id")
    )

    for (guild in c(target_guilds, "netprimprod", "Demersal_fish_larvae")) {
        output_file <- ifelse(
            guild == "netprimprod",
            "ecosystem_indices_",
            "model_outputs_"
        )
        output_extract_func <- ifelse(
            guild == "netprimprod",
            extract_ecosystem_indices_output,
            StrathE2E2:::extract_annual_domain_output
        )
        shap_effects <- e2e_driver_variance_analysis(
            permutation_plan,
            sub_output_path,
            guild,
            output_file_base = output_file,
            output_extraction_func = output_extract_func
        )
        shapley_results[
            shapley_results$output == guild,
        ]$shapley_effect <- shap_effects
    }

    write.csv(
        shapley_results,
        paste0(sub_output_path, "shapley_effects.csv"),
        row.names = FALSE
    )
}
