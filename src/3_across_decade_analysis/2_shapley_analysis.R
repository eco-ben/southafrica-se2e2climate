...
for (esm_ssp in ESM_SSP) {
    ...
    permutation_plan <- read.csv(paste0(output_path, esm_ssp, "_permutation_plan.csv"), col.names = c(variable_group_names, "perm_id"))[1:100, ]
    ...
    write.csv(shapley_results, paste0(output_path, esm_ssp, "_shapley_effects.csv"), row.names = FALSE)
}
