...
for (model_parm in model_parameterisations) {
    model <- e2e_read(
        "South_Africa_MA",
        glue("2010-2019-{model_parm}-ssp126"),
    ...
    for (decade in decades) {
        for (ssp in ssps) {
            sub_output_path <- file.path(
                output_path,
                glue("{model_parm}_parameterisation"),
                ssp,
                decade
            )
            variants_to_permute <- c(
                glue("{decade}-{esms[1]}-{ssp}"),
                glue("{decade}-{esms[2]}-{ssp}")
            )

            permutation_plan <- e2e_driver_permutation_plan(
                model,
                variants_to_permute
            )
            if (is.character(permutation_plan)) {
                for (variable in permutation_plan) {
                    model$data$climate.matching[
                        model$data$climate.matching$Driver == variable,
                    ]$Grouping <- 0
                }
                permutation_plan <- e2e_driver_permutation_plan(
                    model,
            ...
            write.csv(
                permutation_plan,
                paste0(sub_output_path, "permutation_plan.csv"),
                row.names = FALSE
            )

            permutation_sources <- split(
                permutation_plan,
                permutation_plan$perm_id
            )
            ...
            future_map(
                ...
                function(x) {
                    ...
                    assign(
                        "SETUPFILES",
                        setup_files,
                        envir = getNamespace("StrathE2E2")$pkg.env
                    )
                ...
                .options = furrr_options(
                    globals = list(
                        ...
                        sub_output_path = sub_output_path
                    ),
                    packages = "StrathE2E2"
                )
            )
...
for (model_parm in model_parameterisations) {
    ...
    for (decade in decades) {
        for (ssp in ssps) {
            ...
            permutation_plan <- read.csv(file.path(
                sub_output_path,
                "permutation_plan.csv"
            ))
            ...
            future_map(
                ...
                function(x) {
                    ...
                    initial_conditions <- read.csv(file.path(
                        sub_output_path,
                        glue("init_conditions_perm_{unique(x$perm_id)}.csv")
                    ))
                ...
                .options = furrr_options(
                    globals = list(
                        ...
                        setup_files = setup_files,
                        sub_output_path = sub_output_path,
                        extract_ecosystem_indices = extract_ecosystem_indices
                    ),
